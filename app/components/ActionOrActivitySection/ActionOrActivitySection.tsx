...
export default function ActionOrActivitySection({ allActivities, allActions, actionArray, allowedTeachers, activityArray, id }: ActionOrActivitySectionProps) {
    const [currActionsState, setCurrActionsState] = useState<'adding' | 'submit'>('adding');
    const [currActivitiesState, setCurrActivitiesState] = useState<'adding' | 'submit'>('adding');
                    ...
                    actions ?
                        actions?.map((option: OPTION, idx) => (
                            <div key={option?.id} className={`${((idx + 1) !== actionArray.length) && 'border-y border-t-0'} py-6`}>
                                <SingleClassRowOfAction allActions={allActions} allActivities={allActivities} classId={id} allowedTeachers={allowedTeachers} option={option} type={'action'} />
                            </div>
                        ))
                        :
                ...
                </h3>
                {
                    activities ?
                        activities?.map((option: OPTION, idx) => (
                            <div key={option.id} className={`${((idx + 1) !== actionArray.length) && 'border-y border-t-0'} py-6`}>
                                <SingleClassRowOfAction allActions={allActions} allActivities={allActivities} classId={id} allowedTeachers={allowedTeachers} option={option} type={'activity'} />
                            </div >
                        ))
                        :
                    ...
                    (
                        <div className='flex justify-center mt-6'>
                            <button
                                ...
                                type={'button'}
                                className={`m-auto border-y border-x ${currActivitiesState === 'adding' ? 'hover:bg-white border-blue-700 text-white hover:text-blue-700 rounded-lg bg-blue-700' : 'hover:bg-white text-white hover:text-green-700 rounded-lg bg-green-700 border-green-700'} border-x border-y transition-all duration-300 font-semibold py-2 px-4`}
                            >
                                Add Activity
                            </button>
                        </div>
