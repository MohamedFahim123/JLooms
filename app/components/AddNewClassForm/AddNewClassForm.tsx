...
export default function AddNewClassForm({ actions, activities }: AddNewClassFormProps) {
    const { control, handleSubmit, register, clearErrors, reset, setError, formState: { errors } } = useForm<FormDefaultValues>({
        defaultValues: {
            name_en: '',
            actions: [{ id: '' }],
            activities: [{ id: '' }],
    ...
    const handleAddActivity = useCallback(() => appendOnAct({ id: '' }), [appendOnAct]);

    const onSubmit: SubmitHandler<FormDefaultValues> = (data: FormDefaultValues) => {
        const formattedData = {
