...
export default function EmployeesTableRow({ cell }: TableRowProps) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const res = await axios.delete(
          `${dataURLS.deleteEmployee}/${cell?.id}`,
          {
            headers: {
  ...
  return (
    ...
    >
      <td
        className="py-3 px-4"
        onClick={() => router.push(`/dashboard/employees/${cell?.id}`)}
      >
        <Link
          href={`/dashboard/employees/${cell?.id}`}
