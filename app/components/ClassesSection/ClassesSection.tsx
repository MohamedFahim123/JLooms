...
async function fetchClassesData(filters: Record<string, string | number> = {}): Promise<{ data: Table[]; totalPages: number }> {
    loading = true;
    ...
    loading = false;
    return { data: data?.data?.classes || [], totalPages: data?.meta?.totalPages || 1 };
};
export default function ClassesSection() {
    ...
    return (
        <div className={`${totalPages > 1 && 'pb-6'} w-full bg-white shadow-md rounded-lg overflow-hidden`}>
            ...
            <DashBoardFilterations doesNotHaveFilterStatus={true} page="classes" placeHolder="Find a class" />
                    ...
                    loading ?
                        ...
                        classes?.length > 0 ?
                            (
                                ...
                                <div className="flex justify-center min-h-screen">
                                    <p className="text-gray-500 font-semibold pt-10 text-xl">No Classes!</p>
                                </div>
                            )
                }
            </div>
